...
| GET | `/limits` | IP 당 분당 검색 한도·엔진 동시성·대기 시간과 거절 수, 환경변수 기본값 |
| PUT | `/limits` | 한도 저장(즉시 적용). 한도 초과는 429 + `Retry-After`, 엔진 자리 없음은 503 |
...
| `/admin/alerts` | 알림·한도: 웹훅 URL·형식(Slack/JSON)·주기·집계 창·재알림 간격, 규칙별 켜기·임계·최소 표본, 테스트 전송, 지금 평가, **요청 한도·엔진 동시성** 카드, 알림 이력 |
